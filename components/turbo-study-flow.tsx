...
export function TurboStudyFlow({ userDecks }: { userDecks: { id: string, name: string }[] }) {
  ...
  // Chat State
  const [messages, setMessages] = useState<Message[]>([{ role: "model", content: "¡Hola! He analizado tu material. ¿Qué dudas tienes o qué conceptos quieres que te explique?" }]);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
        const base64String = (reader.result as string).split(',')[1]; // Quitar el header data:application/pdf;base64,
  ...
  const handleStartWithYoutube = () => {
      if (!youtubeUrl.includes("youtube.com") && !youtubeUrl.includes("youtu.be")) {
          return toast({ title: "URL inválida", description: "Introduce un enlace válido de YouTube.", variant: "destructive" });
      }
      setIsReady(true);
  };
  ...
  const handleSendMessage = async () => {
      ...
      try {
          ...
          toast({ title: "Error en el chat", description: e.message, variant: "destructive" });
      } finally {
          setIsChatting(false);
      }
  };
  ...
  const handleGenerateTest = async () => {
      ...
      try {
          ...
          toast({ title: "Error al generar test", description: e.message, variant: "destructive" });
          setTestState("setup");
      }
  };
  ...
  // Save to Deck (Reutiliza tu lógica de flashcards)
  const handleSaveToDeck = async () => {
    if (!selectedDeckId) return toast({ title: "Selecciona un mazo", variant: "destructive" });
    setIsSaving(true);
    
    // Adaptamos el formato de las preguntas falladas al formato de tus tarjetas
    const formattedCards = wrongAnswers.map(w => ({
        front: w.question,
        back: `**Respuesta Correcta:** ${w.options[w.answer as keyof typeof w.options]}\n\n*Explicación:* ${w.explanation || "Generado por Turbo Study"}`
    }));
    ...
    try {
      const res = await fetch("/api/save-quiz-cards", {
        ...
        body: JSON.stringify({ 
            ...
            newDeckName: null,
            cards: formattedCards // Asegúrate de que tu endpoint soporte este payload (front y back)
        }),
        headers: { "Content-Type": "application/json" }
      });
      
      if (!res.ok) throw new Error("Fallo al guardar");
      toast({ title: "¡Guardado!", description: "Las tarjetas se han añadido a tu mazo." });
    } catch (e) {
      toast({ title: "Error al guardar tarjetas", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="max-w-2xl mx-auto mt-12 animate-in fade-in slide-in-from-bottom-4">
              <Card className="border-muted shadow-xl">
                  <CardHeader className="text-center">
                      ...
                      <CardDescription className="text-lg">Sube un documento o pega un enlace para empezar a estudiar con IA.</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          >
                              ...
                              <span className="font-semibold">Vídeo de YouTube</span>
                          </div>
                      </div>
                      ...
                      {sourceType === "pdf" && (
                          <div className="border-2 border-dashed rounded-xl p-8 text-center relative hover:bg-muted/30 transition-colors">
                              ...
                              <p className="font-medium">{fileName || "Haz clic o arrastra un PDF aquí"}</p>
                          </div>
                      )}
                      ...
                      {sourceType === "youtube" && (
                          <div className="space-y-4">
                              <Input 
                                  ...
                                  className="h-12"
                              />
                              <Button className="w-full h-12" onClick={handleStartWithYoutube}>Cargar Vídeo</Button>
                          </div>
                      )}
                  </CardContent>
              </Card>
      ...
      <div className="max-w-5xl mx-auto h-[80vh] flex flex-col mt-4 border rounded-xl shadow-lg overflow-hidden bg-background">
          <div className="bg-muted/30 p-4 border-b flex justify-between items-center">
              <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{sourceType === 'pdf' ? fileName : 'Material de YouTube'}</span>
              </div>
              <Button variant="ghost" size="sm" onClick={() => setIsReady(false)}>Cambiar Material</Button>
          </div>
          ...
          <Tabs defaultValue="chat" className="flex-1 flex flex-col h-full overflow-hidden">
              <TabsList className="grid w-full grid-cols-2 rounded-none h-12">
                  <TabsTrigger value="chat" className="text-base">💬 Chat con el Documento</TabsTrigger>
                  <TabsTrigger value="test" className="text-base">📝 Generar Test</TabsTrigger>
              </TabsList>
                  ...
                  <div className="max-w-3xl mx-auto h-full flex flex-col">
                      {testState === "setup" && (
                          <Card className="mt-8 shadow-sm">
                              <CardHeader>
                                  ...
                                  <CardDescription>Extraeremos las preguntas más importantes de tu material.</CardDescription>
                              </CardHeader>
                              <CardContent className="space-y-4">
                                  ...
                                  <Button className="w-full mt-6 h-12 text-base" onClick={handleGenerateTest}>
                                      Generar Test <Sparkles className="ml-2 h-4 w-4" />
                                  </Button>
                              </CardContent>
                          </Card>
                      )}
                      ...
                      {testState === "generating" && (
                          <div className="flex flex-col items-center justify-center h-full gap-4">
                              ...
                              <p className="text-lg font-medium">La IA está leyendo el documento y creando preguntas...</p>
                          </div>
                      )}
                          ...
                          <Card className="shadow-lg border-primary/20 animate-in fade-in slide-in-from-right-4">
                              ...
                              <CardFooter className="justify-end">
                                  <Button size="lg" disabled={!userAnswers[currentQIndex]} onClick={() => {
                                      ...
                                      {currentQIndex === questions.length - 1 ? "Ver Resultados" : "Siguiente"} <ArrowRight className="ml-2 h-4 w-4" />
                                  </Button>
                              </CardFooter>
                          </Card>
                      )}
                      ...
                      {testState === "results" && (
                          <div className="space-y-6 pb-8 animate-in fade-in slide-in-from-bottom-4">
                              <div className="text-center p-8 bg-muted/30 rounded-2xl border">
                                  ...
                                  <p className="text-muted-foreground">Aciertos. ¡Buen trabajo!</p>
                              </div>
                                  ...
                                  <Card className="border-destructive/20 shadow-sm">
                                      <CardHeader className="bg-destructive/5 pb-4 border-b">
                                          <CardTitle className="text-destructive flex items-center gap-2">
                                              <XCircle className="h-5 w-5" /> Repaso de Errores ({wrongAnswers.length})
                                          </CardTitle>
                                      </CardHeader>
                                                      ...
                                                      <div key={i} className="p-4 border rounded-xl bg-background">
                                                          ...
                                                          <div className="space-y-2 text-sm">
                                                              <div className="flex gap-2 text-destructive">
                                                                  ...
                                                                  <span>Tu respuesta: {w.options[w.userAnswer as keyof typeof w.options]}</span>
                                                              </div>
                                                              <div className="flex gap-2 text-green-600">
                                                                  <CheckCircle className="h-4 w-4 shrink-0" />
                                                                  <span>Correcta: {w.options[w.answer as keyof typeof w.options]}</span>
                                                              </div>
                                                              {w.explanation && (
                                      ...
                                      <CardFooter className="bg-muted/10 border-t flex-col sm:flex-row gap-4 justify-between pt-6">
                                          <Select value={selectedDeckId} onValueChange={setSelectedDeckId}>
                                              <SelectTrigger className="w-full sm:w-[250px]"><SelectValue placeholder="Selecciona un mazo para guardar..." /></SelectTrigger>
                                              <SelectContent>
                                                  {userDecks.map(deck => (
                                          ...
                                          <Button onClick={handleSaveToDeck} disabled={isSaving || !selectedDeckId} className="w-full sm:w-auto">
                                              {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Save className="mr-2 h-4 w-4" />} Guardar Errores
                                          </Button>
                                      </CardFooter>
                                  </Card>
                              )}

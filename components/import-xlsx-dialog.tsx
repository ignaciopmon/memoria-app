...
export function ImportXLSXDialog({ deckId }: ImportXLSXDialogProps) {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (filteredData.length === 0) {
            setError("The selected Excel sheet is empty or contains no data.")
            setIsLoading(false)
            return
        }
  ...
  const handleImport = async () => {
    if (!sheetData.length || frontColumn === "" || backColumn === "") {
      setError("Please select the columns for Front and Back")
      return
    }
    ...
    try {
      ...
      // Skip header row if it exists and prepare cards
      const cardsToInsert = sheetData.slice(1).map((row) => ({
        deck_id: deckId,
        front: row[frontIdx] ? row[frontIdx].toString() : "",
        back: row[backIdx] ? row[backIdx].toString() : "",
        ease_factor: 2.5,
        interval: 0,
        repetitions: 0,
      ...
      setError(error instanceof Error ? error.message : "Error importing the cards")
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label htmlFor="xlsx-file">Excel File</Label>
                <div className="flex items-center gap-2">
                  <input id="xlsx-file" type="file" accept=".xlsx" onChange={handleFileChange} className="hidden" />
                  <Button
                    type="button"
                    variant="outline"
                ...
                <>
                  ...
                  <div className="rounded-md border p-3">
                    ...
                    <div className="space-y-2 text-sm">
                      {sheetData.slice(1, 4).map((row, idx) => (
                        <div key={idx} className="rounded-sm bg-muted p-2">
                          <p className="text-xs text-muted-foreground">Front:</p>
                          <p className="mb-1 line-clamp-2">{row[Number.parseInt(frontColumn)] || "(empty)"}</p>
                          <p className="text-xs text-muted-foreground">Back:</p>
                          <p className="line-clamp-2">{row[Number.parseInt(backColumn)] || "(empty)"}</p>
                        </div>
                      ))}
                      {sheetData.length > 4 && (
                        <p className="text-xs text-muted-foreground">
                          ... and {sheetData.length - 4} more card{sheetData.length - 4 !== 1 ? "s" : ""}.
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={isLoading || !sheetData.length || frontColumn === "" || backColumn === ""}
              >
